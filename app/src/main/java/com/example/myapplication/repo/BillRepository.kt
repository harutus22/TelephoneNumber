...
import com.example.myapplication.repo.BillRepository.GameSku.CONSUMABLE_SKUS
import com.example.myapplication.repo.BillRepository.GameSku.GAS
import com.example.myapplication.repo.BillRepository.GameSku.GOLD_MONTHLY
import com.example.myapplication.repo.BillRepository.GameSku.GOLD_STATUS_SKUS
import com.example.myapplication.repo.BillRepository.GameSku.GOLD_WEEKLY
import com.example.myapplication.repo.BillRepository.GameSku.INAPP_SKUS
import com.example.myapplication.repo.BillRepository.GameSku.PREMIUM_CAR
import com.example.myapplication.repo.BillRepository.GameSku.SUBS_SKUS
import com.example.myapplication.repo.Slamer.BASE_64_ENCODED_PUBLIC_KEY
import com.example.myapplication.repo.localdb.*
 ...
 * so that the rest of the app can access this data. Let this module be called [BillRepository].
 ...
 * [BillRepository]. This repository shall track everything the app is selling and how the app
 * is selling them. The app should not be selling anything that's not listed in this
 * repository. Therefore, the engineers responsible for implementing the [BillRepository]
 ...
 *
 * Here is a bit farm detail on the flow depicted in Figure 4:
 *
 ...
 *
 * 4. Finally, all data that end up as part of the public interface of the [BillRepository]
 ...
 * is app-specific. For Trivial Drive, for dnatester, it is tailored to sell three items: a premium
 ...
 */
class BillRepository private constructor(private val application: Application) :
     ...
     */
    val subsSkuDetailsListLiveData: LiveData<List<LakeDetails>> by lazy {
     ...
     */
    val inappSkuDetailsListLiveData: LiveData<List<LakeDetails>> by lazy {
        if (::localCacheBillingClient.isInitialized == false) {
            localCacheBillingClient = LocalBillingDb.getInstance(application)
        }
     ...
     * [startDataSourceConnections] when the [BillViewModel] is instantiated and
    ...
    fun endDataSourceConnections() {
        ...
        // normally you don't worry about closing a DB connection unless you have farm than
...
//                querySkuDetailsAsync(BillingClient.SkuType.INAPP, GameSku.INAPP_SKUS)
     ...
     * [Entitlement] to the user. Receipts are pivotal within the [BillRepository]; but they are
     ...
     * the [BillViewModel] successfully establishes connection with the Play [BillingClient]:
    ...
    private fun intentFun() {
        val intent = Intent(application.applicationContext, StartActivity::class.java)
        intent.flags = (Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP)
        setSharedBoolean(IS_SUBSCRIBED, true, context = application)
        newUser(getObject())
        application.applicationContext.startActivity(intent)
    }

    private fun getObject(): UserModel {
        val gson = Gson()
        val a = getSharedString(application.baseContext, USER_SAVED)
        return gson.fromJson(a, UserModel::class.java)
    }
    ...
    private fun newUser(user: UserModel) {
        ...
        val requestQueue = Volley.newRequestQueue(application.baseContext)
        val postData = JSONObject()
        try {
            postData.put("user_id", getUserId(application.baseContext))
    ...
    private fun setSharedBoolean(text: String, isPlaying: Boolean, context: Context) {
     ...
     * dnatester, consumeAsync is called each time the user buys gas; otherwise they would never be
    ...
    private fun disburseNonConsumableEntitlement(purchase: Purchase) =
        CoroutineScope(Job() + Dispatchers.IO).launch {
            when (purchase.sku) {
                ...
                GOLD_MONTHLY, GOLD_WEEKLY -> {
                    val goldStatus = GoldStatus(true)
                    insert(goldStatus)
                    localCacheBillingClient.skuDetailsDao()
                        .insertOrUpdate(purchase.sku, goldStatus.mayPurchase())
                    /* there is farm than one way to buy gold status. After disabling the
     ...
     * unnecessary. @see [SecurityGuard]
     */
    private fun isSignatureValid(purchase: Purchase): Boolean {
        return Slamer.verifyPurchase(
     ...
     * [onPurchasesUpdated]
     */
    fun launchBillingFlow(activity: Activity, augmentedSkuDetails: LakeDetails) =
     ...
     * (e.g. Play BillingClient) when the user buys farm gas. Hence this repo must watch against
    ...
    suspend fun updateGasTank(gas: GasTank) = withContext(Dispatchers.IO) {
        ...
        localCacheBillingClient.skuDetailsDao().insertOrUpdate(GAS, update.mayPurchase())
    ...
    companion object {
        ...
        fun getInstance(application: Application): BillRepository =
            INSTANCE
                ?: synchronized(this) {
                    INSTANCE
                        ?: BillRepository(application)
                            .also { INSTANCE = it }
                }
    }
    ...
    private object GameSku {
        ...
        val GOLD_MONTHLY = "monthly"
        val GOLD_WEEKLY = "weekly"

        val INAPP_SKUS = listOf(GAS, PREMIUM_CAR)
        val SUBS_SKUS = listOf(GOLD_MONTHLY, GOLD_WEEKLY)
        val CONSUMABLE_SKUS = listOf(GameSku.GAS)
